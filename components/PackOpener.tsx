...
import { generatePackContents } from '../services/geminiService';
import { Sparkles, Loader2, ChevronLeft, ChevronRight, Coins, AlertCircle, RefreshCw } from 'lucide-react';
...
const PackOpener: React.FC<PackOpenerProps> = ({ onCardsRevealed, coins, onSpendCoins }) => {
  ...
  // Fix: Correctly initialize useState for revealIndex
  const [revealIndex, setRevealIndex] = useState(-1);
  const [currentPackIndex, setCurrentPackIndex] = useState(0);
  
  // New state for card transition animation
  const [displayCard, setDisplayCard] = useState<Card | null>(null);
  const [isAnimatingOut, setIsAnimatingOut] = useState(false);

  // packImageCache and imageLoading removed

  const packs = [
    {
      id: 'legendary-buck',
      name: 'Legendary Buck',
      subtitle: 'Il pack dei felini', // New subtitle
      theme: 'Un gatto arancione saggio con un occhio solo seguito da un esercito di squali, cani, zanzare e uccelli.',
      // imagePrompt removed
    }
  ];

  // fetchPackImage useEffect and function removed

  // Effect to update the displayed card when revealIndex changes
  useEffect(() => {
    if (step === 'revealed' && openedCards.length > 0 && revealIndex !== -1) {
      setDisplayCard(openedCards[revealIndex]);
    }
  }, [step, revealIndex, openedCards]);

  const nextPack = () => {
    if (loading) return; // Removed imageLoading from condition
  ...
  const openPack = async () => {
    if (coins < PACK_COST) return; // Removed hasApiKeySelected condition
  ...
  const nextCard = () => {
    if (revealIndex < openedCards.length - 1) {
      setIsAnimatingOut(true); // Trigger exit animation for current card
      setTimeout(() => {
        setRevealIndex(prev => prev + 1); // Move to next card after exit animation
        setIsAnimatingOut(false); // Reset animation state, new card will animate in via useEffect
      }, 300); // Match this duration to the 'animate-out' class duration
    } else {
      const finalCards = [...openedCards];
      setOpenedCards([]);
      setRevealIndex(-1);
      setIsAnimatingOut(false); // Ensure reset
      setDisplayCard(null); // Clear display card on completion
      setIsGoldPack(false);
      setStep('idle');
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] py-8 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-4 sm:gap-12 mb-12">
          <button 
            onClick={prevPack}
            disabled={loading} // Removed imageLoading from disabled
          ...
          <div className="relative group perspective-1000">
            <div className={`absolute -inset-6 bg-indigo-500/10 rounded-full blur-3xl group-hover:bg-indigo-500/20 transition-all duration-700 ${loading ? 'animate-pulse' : ''}`} />
            ...
            <div className="relative w-64 h-[400px] rounded-xl shadow-2xl overflow-hidden border-2 border-white/10 group-hover:scale-105 transition-transform duration-500 animate-float bg-slate-900 flex items-center justify-center">
               {/* Replaced image display with text-based representation */}
               {error ? (
                 <div className="flex flex-col items-center gap-3 p-6 text-center">
                   <AlertCircle className="text-red-500" size={32} />
                   <p className="text-slate-400 text-[10px] font-bold uppercase">{error}</p>
                   <button 
                    onClick={() => setError(null)} // Simplified retry, as no image fetching
                 ...
                 <div className="flex flex-col items-center justify-center h-full w-full bg-slate-800 p-4 text-center">
                   <Sparkles className="text-white/30 mb-4" size={48} />
                   <p className="font-cinzel text-lg text-white leading-tight uppercase tracking-tighter drop-shadow-lg">{packs[currentPackIndex].name}</p>
                   <p className="text-slate-500 text-xs mt-2 italic">"{packs[currentPackIndex].theme.substring(0, 50)}..."</p>
                 </div>
               )}
               ...
               <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/80 to-transparent p-6 pt-12">
                 ...
                 <p className="text-xs text-indigo-300 mb-2 font-bold uppercase tracking-wider text-center">{packs[currentPackIndex].subtitle}</p> {/* Display subtitle */}
        ...
        <div className="w-64 space-y-4 text-center">
          ...
          <button 
            disabled={loading || !!error || coins < PACK_COST} // Removed hasApiKeySelected from disabled condition
            onClick={openPack}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[75vh] p-4 relative overflow-hidden">
      ...
      {displayCard && ( // Use displayCard here
        <div className="flex flex-col items-center">
          ...
          <div 
            key={displayCard.id} // Important for React to manage transitions
            className={`${isAnimatingOut ? 'animate-out fade-out zoom-out duration-300' : 'animate-in fade-in zoom-in duration-500'}`}
          >
            <CardComponent card={displayCard} />
          </div>
          <button 
            onClick={nextCard}
            disabled={isAnimatingOut} // Disable button during animation
          ...
          >
            {isAnimatingOut ? <Loader2 className="animate-spin" size={20} /> : (revealIndex === openedCards.length - 1 ? 'Salva nell\'Album' : 'Prossima Carta')}
          </button>
        </div>
      )}
    </div>
